...
### HOM_DUP ##############################################################################
calc_p_homDup_wc = function(df, alpha = 0.05) {
  r_w = df$scalar * df$expected * df$nb_p /(1-df$nb_p)
  r_c = r_w
  return(log(dnbinom(df$W, r_w, df$nb_p)) + log(dnbinom(df$C, r_c, df$nb_p)))
}
calc_p_homDup_ww = function(df, alpha = 0.05) {
  r_w = df$scalar * df$expected * df$nb_p /(1-df$nb_p) * (2-alpha)
  r_c = df$scalar * df$expected * df$nb_p /(1-df$nb_p) * alpha
  return(log(dnbinom(df$W, r_w, df$nb_p)) + log(dnbinom(df$C, r_c, df$nb_p)))
}
calc_p_homDup_cc = function(df, alpha = 0.05) {
  r_w = df$scalar * df$expected * df$nb_p /(1-df$nb_p) * alpha
  r_c = df$scalar * df$expected * df$nb_p /(1-df$nb_p) * (2-alpha)
  return(log(dnbinom(df$W, r_w, df$nb_p)) + log(dnbinom(df$C, r_c, df$nb_p)))
}

add_p_homDup <- function(df, alpha = 0.05) {
  df$p_homDup = MAX_NEGATIVE_VALUE
  df[state == "WW"]$p_homDup = calc_p_homDup_ww(df[state == "WW"], alpha)
  df[state == "WC"]$p_homDup = calc_p_homDup_wc(df[state == "WC"], alpha)
  df[state == "CW"]$p_homDup = calc_p_homDup_wc(df[state == "CW"], alpha)
  df[state == "CC"]$p_homDup = calc_p_homDup_cc(df[state == "CC"], alpha)
  df[p_homDup < MAX_NEGATIVE_VALUE]$p_homDup = MAX_NEGATIVE_VALUE
  df
}
...
### HET_IDUP #############################################################################
calc_p_hetIdup_wc = function(df, alpha = 0.05) {
  return (calc_p_hetDup_wc(df, alpha))
}
calc_p_hetIdup_ww = function(df, alpha = 0.05) {
  r_w = df$scalar * df$expected * df$nb_p /(1-df$nb_p) * (1-alpha)
  r_c = df$scalar * df$expected * df$nb_p /(1-df$nb_p) / 2
  return(log(dnbinom(df$W, r_w, df$nb_p)) + log(dnbinom(df$C, r_c, df$nb_p)))
}
calc_p_hetIdup_cc = function(df, alpha = 0.05) {
  r_w = df$scalar * df$expected * df$nb_p /(1-df$nb_p) / 2
  r_c = df$scalar * df$expected * df$nb_p /(1-df$nb_p) * (1-alpha)
  return(log(dnbinom(df$W, r_w, df$nb_p)) + log(dnbinom(df$C, r_c, df$nb_p)))
}

add_p_hetIdup <- function(df, alpha = 0.05) {
  df$p_hetIdup = MAX_NEGATIVE_VALUE
  df[state == "WW"]$p_hetIdup = calc_p_hetIdup_ww(df[state == "WW"], alpha)
  df[state == "WC"]$p_hetIdup = calc_p_hetIdup_wc(df[state == "WC"], alpha)
  df[state == "CW"]$p_hetIdup = calc_p_hetIdup_wc(df[state == "CW"], alpha)
  df[state == "CC"]$p_hetIdup = calc_p_hetIdup_cc(df[state == "CC"], alpha)
  df[p_hetIdup < MAX_NEGATIVE_VALUE]$p_hetIdup = MAX_NEGATIVE_VALUE
  df
}
...
add_NB_probs <- function(df, alpha = 0.05) {
  ...
  df <- add_p_homDup(df, alpha)
  df <- add_p_hetIdup(df, alpha)
  df
}